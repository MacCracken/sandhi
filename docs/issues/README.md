...
|-----|-------|--------|---------|
| [`2026-06-14-agnos-socket-backend-gap.md`](2026-06-14-agnos-socket-backend-gap.md) | 2026-06-14 | compile gap ✅ 1.5.1 (Batch C1) | The HTTP client's timeout-bounded non-blocking connect (`src/http/conn.cyr`) and the async server's non-blocking listen fd (`src/server/mod.cyr`) dropped to raw Linux socket syscalls (`SYS_SOCKET`/`SYS_CONNECT`/`SYS_FCNTL`/`SYS_SETSOCKOPT`/`SOL_SOCKET`) undefined on AGNOS, so `cyrius build --agnos` of any consumer (sit) failed to **compile**. **Fixed in 1.5.1**: every raw-syscall site guarded by `#ifndef CYRIUS_TARGET_AGNOS` (agnos counterpart under `#ifdef`) — nb-connect → blocking `sock_connect`, SO_*TIMEO → no-op, v6 → fail-closed (IPv4-only), listen-fd fcntl compiled out. Linux/macOS proven byte-identical. Remaining (NOT C1 scope): `resolve.cyr` `/dev/urandom` entropy (runtime; roadmap C2), upstream `mmap` `CLONE_VM` stub, native `SSL_CTX_*` on agnos (A1). The transport half of **sit adoption on AGNOS**. |
