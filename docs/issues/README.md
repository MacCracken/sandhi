...
|-----|-------|--------|---------|
| [`2026-06-14-agnos-socket-backend-gap.md`](2026-06-14-agnos-socket-backend-gap.md) | 2026-06-14 | OPEN — 1.5.x (Batch C lead) | The HTTP client's timeout-bounded non-blocking connect (`src/http/conn.cyr`) and the async server's non-blocking listen fd (`src/server/mod.cyr`) drop to raw Linux socket syscalls (`SYS_SOCKET`/`SYS_CONNECT`/`SYS_FCNTL`/`SYS_POLL`/`SYS_GETSOCKOPT`/`SYS_SETSOCKOPT`) that AGNOS doesn't define, so `cyrius build --agnos` of any sandhi consumer (sit) fails to **compile**. TLS + DNS + mDNS are already portable. Bounded fix: one `_sandhi_connect_bounded_a` transport seam with a Linux impl (current machinery) + an agnos impl (blocking `sock_connect`, v6 fails cleanly, listen-fd nonblock skipped). **Needs the cyrius ≥6.2.6 pin (delivered at 1.5.0).** This is the transport half of **sit adoption on AGNOS** — the first concrete sit-driven 1.5.x item. |
