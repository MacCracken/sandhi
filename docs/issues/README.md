...
|-----|-------|--------|---------|
| [`2026-06-14-agnos-socket-backend-gap.md`](2026-06-14-agnos-socket-backend-gap.md) | 2026-06-14 | sandhi-side ✅ (C1 1.5.1 + C2 1.5.2) | sandhi's transport layer used raw Linux syscalls undefined on AGNOS. **C1 (1.5.1)** — the bounded nb-connect (`src/http/conn.cyr`) + async listen fd (`src/server/mod.cyr`) `SYS_*`/`SOL_SOCKET` sites failed to **compile** for `--agnos`; fixed by `#ifndef CYRIUS_TARGET_AGNOS` guards (nb-connect → blocking `sock_connect`, SO_*TIMEO → no-op, v6 → fail-closed, listen-fd fcntl out), Linux/macOS byte-identical. **C2 (1.5.2)** — the DNS TXID seed (`resolve.cyr`) used `/dev/urandom` via bare Linux syscall numbers (compiled but wrong at *runtime* on agnos); fixed by composing the portable stdlib `sys_getrandom` selector primitive (AGNOS #45 / Linux / macOS / Windows; no `#ifdef`, no new dep). **Sandhi-side AGNOS transport complete.** A full `--agnos` build needs a *cascade* of stdlib agnos-compile gaps cleared (see the cascade filing below — part sandhi-side dep-snapshot refresh, part upstream) + native `SSL_CTX_*` (A1). The transport half of **sit adoption on AGNOS**. |

Resolved sandhi-side defects are in the [Archived](#archived-resolved)
table below — the 1.4.x arc closed the HTTP close-path drain (1.4.1), the
repeated-HTTPS-request SIGSEGV (1.4.5), the high-level client TLS-policy
threading gap (1.4.6), and the low-level TLS-policy-enforcement live SIGSEGV
(1.4.7); 1.5.0 archived the batch. New sandhi-side defects land here as
`YYYY-MM-DD-kebab-case.md` and move to `archive/` when closed.

## Upstream dependencies (sandhi is blocked on stdlib / toolchain)

| Doc | Filed | Status | Summary |
|-----|-------|--------|---------|
| [`2026-05-22-cyrius-native-tls-in-6.0.x.md`](2026-05-22-cyrius-native-tls-in-6.0.x.md) | 2026-05-22 | Open — substantially landed | `lib/tls.cyr` native-TLS swap (off the fdlopen-libssl bridge). **Native transport operational + the no-flag default since 1.4.2 / cyrius 6.1.21** — so the **sit-adoption gate is cleared**. Residual before full close: typed native `SSL_CTX_*` equivalents (pre-handshake trust-store / mTLS *enforcement* — native fails closed without them at 1.4.7), tracked as a cross-repo dependency in [roadmap.md](../development/roadmap.md). Sandhi-side contract unchanged per ADR 0001 + CLAUDE.md "No FFI". |
...
| [`2026-06-15-cyrius-mdns-multicast-primitives.md`](2026-06-15-cyrius-mdns-multicast-primitives.md) | 2026-06-15 | Open — cyrius-side (quality gate) | cyrius `lib/net.cyr` ships no IPv4 multicast primitives — no `IP_ADD_MEMBERSHIP` / `IP_MULTICAST_TTL` / `_LOOP` / `_IF` / `SO_REUSEPORT`, no `ip_mreq` struct, no join helper (only the generic `sys_setsockopt`). Gates sandhi's `discovery/local.cyr` real multicast (QM-mode + RFC 6763 service browsing). **Not a hard blocker**: the 0.9.3 QU-bit unicast resolver ships today and needs none of these. Filing carries the exact constants / `ip_mreq` shape / preferred `net_join_multicast` helper. Batch A3; the last untracked upstream item in Batch A (filed 1.5.3). |
| [`2026-06-15-cyrius-thread-agnos-clone-dispatch.md`](2026-06-15-cyrius-thread-agnos-clone-dispatch.md) | 2026-06-15 | Open — mixed (sandhi-side + upstream) | **AGNOS full-build cascade.** Not a single defect — `cyrius build --agnos` of a sandhi consumer trips stdlib agnos-compile gaps one at a time. **(1) sandhi-side**: the vendored `./lib/thread.cyr` is a stale pre-fix snapshot — its agnos dispatch (`→ thread_agnos.cyr`) is **already in the 6.2.6 toolchain**, so this is a dep-snapshot refresh, not an upstream defect (the `mmap.cyr:184` `CLONE_VM` error is an include-offset artifact for `thread.cyr:199`). **(2) upstream**: refreshing `thread.cyr` exposes `async.cyr`'s raw `SYS_EPOLL_CREATE1` (agnos has `SYS_EPOLL_CREATE` + the portable `sys_epoll_create` wrapper, not the `1` variant) — a real gap; more layers unenumerated. Needs a systematic stdlib agnos-completeness pass + a vendored-snapshot refresh. Supersedes the earlier "`mmap` stub" / "upstream `thread.cyr` defect" framings (an adversarial verification pass corrected both). None gates sandhi's authoritative x86_64 artifacts. |
