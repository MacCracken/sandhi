...
| [`2026-06-09-https-repeated-request-segfault.md`](2026-06-09-https-repeated-request-segfault.md) | 2026-06-09 | ✅ Resolved 1.4.5 | `sandhi_http_get`/`_post` SIGSEGV on the ~4th sequential HTTPS request to the same host. Root cause was upstream — cyrius `alloc.cyr` `brk` heap × glibc-malloc contention via `fdlopen`-libssl. **Fixed in 1.4.5 / cyrius 6.1.19**: alloc heap → anonymous-mmap; sandhi default-switched to native TLS. Surfaced by hoosh v2.2.0. |
| [`2026-06-09-https-client-tls-policy-threading.md`](2026-06-09-https-client-tls-policy-threading.md) | 2026-06-09 | ✅ Resolved 1.4.6 | High-level client (`sandhi_http_*` / `sandhi_http_stream`) couldn't carry a TLS policy: `sandhi_http_options` had no policy field and the request path opened via plain `sandhi_conn_open_fully_timed_a`. **Fixed in 1.4.6**: `sandhi_http_options_tls_policy` + getter; HTTPS open bracketed by the policy pre/post-open helpers (fail-closed, post-handshake SPKI pin, pool/0-RTT bypass). Live gate `_https_policy_threading_gate.cyr` green. Surfaced by hoosh v2.2.0. |
| [`2026-06-09-tls-policy-enforcement-live-segfault.md`](2026-06-09-tls-policy-enforcement-live-segfault.md) | 2026-06-09 | Open — **P2** | Low-level `sandhi_conn_open_with_policy` SIGSEGVs on a LIVE network in the still-libssl-coupled enforcement (libssl SPKI read on the libssl backend at gate [3]; native trust-store `SSL_CTX_*` config at gate [4]). `enforcement_available()` reports 1 on native but applying libssl `SSL_CTX_*` to a native ctx faults. **Pre-exists 1.4.6** (reproduces on pristine `main`); offline CI skip-cleans. Tied to the "native TLS-policy enforcement" libssl-retirement gate. |
