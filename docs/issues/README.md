...
| [`2026-06-15-cyrius-net-v6-darwin.md`](2026-06-15-cyrius-net-v6-darwin.md) | 2026-06-15 | 🔵 Open (cyrius `lib/net.cyr`) | stdlib's IPv6 surface isn't Darwin-ported — `SockDomain.AF_INET6=10` unbranched (Darwin=30), no `sockaddr_in6` builder, no v6 `net_connect_nb`, no `sock_set_nonblocking` helper. Blocks the v6 + server-listen-socket follow-on to the 1.6.1 macOS connect fix; sandhi can't compose what stdlib doesn't expose (ADR 0001 / No-FFI), so filed upstream. Low severity (v6 connect masked by v4 fallback). |
