...
| [`2026-06-09-tls-policy-enforcement-live-segfault.md`](2026-06-09-tls-policy-enforcement-live-segfault.md) | 2026-06-09 | ✅ SIGSEGV resolved 1.4.7 | Low-level `sandhi_conn_open_with_policy` SIGSEGV'd on a LIVE network (native trust-store fed a native ctx to libssl `SSL_CTX_*`; libssl pinning hit a cyrius `tls_get_peer_spki_der` regression). **Fixed in 1.4.7**: `enforcement_available()` is backend-aware (trust/mTLS → 0 on native) + new `pin_available()` (SPKI, backend-agnostic, libssl-excluded); `_sandhi_policy_pre_open_a` fails closed before the faulting paths. Native gates ALL PASS, no crash. Enforcement-parity follow-ups (native `SSL_CTX_*`; cyrius libssl-SPKI fix) tracked cross-repo. |
