...
| [`2026-06-15-cyrius-thread-agnos-clone-dispatch.md`](2026-06-15-cyrius-thread-agnos-clone-dispatch.md) | 2026-06-15 | Open — cyrius-side | `lib/thread.cyr`'s thread-spawn `clone(2)` flags (`CLONE_VM | …`, `thread.cyr:199`) are emitted unconditionally inside `#ifndef CYRIUS_TARGET_WIN`, but the agnos syscall table defines no `clone` constants, so `cyrius build --agnos` of any `thread`-pulling consumer (sandhi included) aborts `undefined variable 'CLONE_VM'` (reported at `mmap.cyr:184` — an include-offset artifact; the token lives in `thread.cyr`). A `thread_agnos.cyr` peer exists in the toolchain but is neither wired into `thread.cyr`'s dispatch (only WIN is) nor vendored. Fix: route AGNOS to its peer like WIN. The corrected, precise filing for what 1.5.1/1.5.2 imprecisely called the "`mmap` `CLONE_VM` stub" — one of two remaining full-`--agnos` blockers (with native `SSL_CTX_*`). |
