...
|-----|-------|--------|---------|
| [`2026-05-22-cyrius-native-tls-in-6.0.x.md`](2026-05-22-cyrius-native-tls-in-6.0.x.md) | 2026-05-22 | Open — substantially landed | `lib/tls.cyr` native-TLS swap (off the fdlopen-libssl bridge). **Native transport operational + the no-flag default since 1.4.2 / cyrius 6.1.21**; the sit-adoption gate is cleared. Residual before full close: typed native `SSL_CTX_*` equivalents (pre-handshake trust-store / mTLS *enforcement* — native fails closed without them at 1.4.7). **Re-verified still-open on cyrius 6.2.7** (only `tls_set_verify` exists; no native trust-store / client-cert / client-key verb). The last open Batch-A item. |
...
| [`2026-06-15-cyrius-mdns-multicast-primitives.md`](2026-06-15-cyrius-mdns-multicast-primitives.md) | 2026-06-15 | Open — partially landed (Batch A3) | cyrius 6.2.7 shipped the IPv4 multicast **join/option** primitives sandhi filed (`net_join_multicast` / `sock_reuseport` / `net_set_multicast_ttl/loop/if` + `IP_MULTICAST_*` / `SO_REUSEPORT` / `ip_mreq`) — **but they're insufficient.** A 1.5.4 QM-resolver adoption was reverted after review found that `net.cyr`'s connected-socket `sock_send`/`sock_recv` force a `connect()` whose Linux source-filter drops mDNS answers. Needs unconnected `sock_sendto`/`sock_recvfrom` in `net.cyr` (or a two-socket split) + a loopback live test. **Not a hard blocker** (QU is the default — though it shares the same connect()-filter exposure; needs a live check). |
