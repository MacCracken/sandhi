...
- **0.9.6** ✅ (this release) — ALPN-driven h2 auto-promotion via `_sandhi_http_try_h2_promote`: open advertising `h2,http/1.1`, branch on `sandhi_conn_alpn_is_h2` (h2 → preface/SETTINGS/cache; 1.1 → donate to pool's 1.1 slot via `_sandhi_pool_has_idle`-gated put). First release where live h2 fires end-to-end via the auto path.
