...
**1.5.1** — 2026-06-15. **1.5.x Batch C1 — AGNOS socket-backend gap (sit-adoption-driven; no pin change, stays 6.2.6).** sandhi's transport layer dropped to raw Linux socket syscalls (`SYS_FCNTL` / `SYS_SOCKET` / `SYS_CONNECT` / `SYS_SETSOCKOPT` / `SOL_SOCKET`) undefined on the AGNOS target — the HTTP-client bounded non-blocking connect (`src/http/conn.cyr`) and the async-server non-blocking listen fd (`src/server/mod.cyr`) — so `cyrius build --agnos` of any consumer (sit) failed to **compile** (`undefined variable 'SYS_FCNTL'`). Every site is now wrapped in `#ifndef CYRIUS_TARGET_AGNOS` with an agnos counterpart under `#ifdef` (stdlib `chrono`/`net` precedent): nb-connect → blocking `sock_connect` (timeout advisory, = the `connect_ms==0` path); per-op SO_RCVTIMEO/SNDTIMEO → no-op; IPv4-only v6 paths → fail-closed (`SANDHI_CONN_OPEN_CONNECT`); server listen-fd `O_NONBLOCK` fcntl compiled out (`sock_listen` already `Err`s on agnos). **Linux/macOS proven byte-identical** — pre/post `programs/smoke.cyr` `cmp`-clean (the `#ifndef` blocks emit identical code). Validated: standalone agnos probe (guarded `SYS_FCNTL` compiles `--agnos`; unguarded reproduces the error), 26-site structural sweep (all guarded), **992 assertions green** (440 + 167 + 343 + 42), aarch64 cross-build still green, lint 0/0, `dist/sandhi.cyr` regenerated at v1.5.1. **Remaining agnos-build blockers (NOT C1 scope, tracked):** `resolve.cyr` `/dev/urandom` DNS entropy (runtime; roadmap C2), upstream `lib/mmap.cyr` `CLONE_VM` stub, native `SSL_CTX_*` for the bundle's `fdlopen`/`tls_dlsym` path (Batch A1).
...
**992 assertions green** across four suites (CI runs all four; measured on the 6.2.6 pin at 1.5.1):
...
- **Batch C — sit-adoption reshape** — gate **cleared** (native TLS default
  since 6.1.21). **C1 AGNOS socket-backend gap — ✅ shipped at 1.5.1**: sandhi's
  raw-Linux-syscall connect/listen machinery (`src/http/conn.cyr`,
  `src/server/mod.cyr`) now compiles for `--agnos` via `#ifndef CYRIUS_TARGET_AGNOS`
  guards (Linux/macOS byte-identical). **C2 AGNOS DNS-entropy** — queued: the
  `/dev/urandom` TXID seed in `resolve.cyr` compiles on agnos but is a latent
  runtime gap (agnos sources entropy via syscall); opens when the agnos
  entropy-syscall surface firms up. Further Batch C items fill from what sit
  surfaces, not pre-baked (memory [`project_sit_adoption_drives_roadmap`]). Full
  `--agnos` build also needs the upstream `mmap` `CLONE_VM` stub + native
  `SSL_CTX_*` (A1).
