...
**Unreleased earlier** — Bites 1 + 2 + 2.5 + 3 + 4 + 2b + 5 (a/b/c) landed. h2 protocol stack is functionally complete end-to-end. **Bite 5b — request send** ~170 lines (`src/http/h2/request.cyr`). HPACK-encoded HEADERS block (4 pseudo-headers in spec order, lowercased user headers, forbidden-list filter), HEADERS+optional-DATA frame send. **Bite 5c — response decode** ~210 lines (`src/http/h2/response.cyr`). Frame-loop dispatcher for SETTINGS/PING/GOAWAY/RST_STREAM/HEADERS/CONTINUATION/DATA, PADDED flag honored, pseudo-headers stripped, returns the same `sandhi_response` struct as the 1.1 path. **592 total assertions (446 sandhi + 146 h2; +23 across 5b + 5c).** Bites 6 and 7 still ahead — Bite 6 multiplex via pool, Bite 7 auto-selection on ALPN + 0.8.0 release wrap.

**Unreleased earlier** — Bites 1 + 2 + 2.5 + 3 + 4 + 2b + 5a landed. **Bite 5a — h2 connection lifecycle scaffolding** ~210 lines in `src/http/h2/conn.cyr`. h2 conn struct (80 bytes; HPACK enc + dec tables, peer settings, stream-id counter), frame send/recv plumbing, preface + SETTINGS handshake. Bite 5 split into 5a/5b/5c because the full lifecycle is one big lift; 5b adds HEADERS encoding, 5c adds response-decoding + stream state machine. **569 total assertions (446 sandhi + 123 h2; +12 conn).** Bites 5b, 5c, 6, 7 still ahead.

**Unreleased earlier** — Bites 1 + 2 + 2.5 + 3 + 4 + 2b landed. **Bite 2b — HPACK Huffman decode** ~150 lines + 2570-char data blob in `src/http/h2/huffman.cyr`. Full RFC 7541 Appendix B table embedded as one fixup; lazy-init binary tree; padding-rule enforcement per §5.2 (≤7 bits, all-1s). HPACK's `_hpack_string_decode` no longer rejects H=1 input — round-trips through the new decoder. RFC C.4.1 `www.example.com` verified. Real h2 server compatibility unblocked. **557 total assertions (446 sandhi + 111 h2; +4 Huffman).** Bite 5 (h2 connection lifecycle) now technically unblocked — that's the next big lift. Bites 6, 7 still ahead.
...
- **0.8.1** ✅ (this release) — `sandhi_http_request_auto` + per-method auto verbs. Pool h2-take → 1.1 fallback. ALPN-advertise upstream-ask filed.
- **0.9.0** — Phase 1 security sweep (5 P0 fixes from the 0.7.0 audit): chunked truncation guard, CL+TE coexistence reject, chunk-size overflow, redirect credential strip, TLS-policy fail-closed.
- **0.9.1** — Phase 2 P1 sweep (~7 items): SSE re-entrance, header dup-detect, SPKI constant-time compare, CL strict parse, URL port overflow, JSON escape state, header CRLF validation.
