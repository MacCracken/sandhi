...
**1.6.3** — 2026-06-15. **WebDriver/Appium/MCP RPC can now carry a TLS policy — endpoint-keyed default policy registry; cyrius pin `6.2.10 → 6.2.11`.** Closes [`archive/2026-06-15-yantra-sandhi-wd-rpc-no-tls-policy.md`](../issues/archive/2026-06-15-yantra-sandhi-wd-rpc-no-tls-policy.md) (yantra M8). The RPC convenience verbs (`sandhi_wd_*` / `sandhi_ap_*` / `sandhi_rpc_mcp_*`) took only a `base_url` with no options, so a remote HTTPS grid could pin only the session-create POST yantra issued itself — every per-action call fell back to default trust. **Resolution (option (2) from the filing):** a process-wide base-URL-keyed default policy registry at the shared dispatch layer (`src/rpc/dispatch.cyr`), +4 public verbs — `sandhi_rpc_set_default_tls_policy(base_url, policy)` / `_clear_default_tls_policy` / `_get_default_tls_policy` / `_clear_all_default_tls_policy`. Per-call resolution is **longest-prefix match with a path boundary** (so `host:444` ≠ `host:4444/…`); every RPC call whose URL falls under a registered `base_url` opens through the policy with the same pin/mTLS/trust-store semantics as `sandhi_http_options_tls_policy` (conn not pooled). `_sandhi_rpc_http_send` reworked into `_sandhi_rpc_http_send_a` routing through `_sandhi_http_dispatch_a` (opts==0 → byte-identical to pre-1.6.3); `sandhi_rpc_mcp_stream_a` carries the policy onto the SSE channel via `sandhi_http_stream_opts_a`. Plain-HTTP `127.0.0.1` backends unaffected (HTTP layer ignores policy on non-TLS). Registry stored in `default_alloc()` (keys + policies outlive request arenas, like the session cache). No new cyrius primitive needed — pure composition of the existing TLS-policy + dispatch surface. **Verified**: **1023 assertions green** (451 + 167 + 342 + **63**, rpc 42 → 63: registry set/get/replace, longest-prefix + boundary guard, scheme-agnostic resolve, clear/clear-all/nested-survives-sibling-clear, 16-endpoint cap + replace-at-cap); lint 0/0; `cyrfmt` clean; `dist/sandhi.cyr` regenerated at v1.6.3.
...
**1023 assertions green** across four suites (CI runs all four; measured on the 6.2.11 pin at 1.6.3):
