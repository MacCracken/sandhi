...
- **0.8.0** ✅ (this release) — HTTP/2 + connection pool (8 bites). Pool + 1.1 keep-alive, full HPACK + Huffman decode, h2 frames + lifecycle, ALPN surface (runtime stubbed), public `sandhi_h2_request` verb. Auto-selection in `sandhi_http_get` deferred to 0.8.1 alongside libssl-pthread-deadlock fix.
