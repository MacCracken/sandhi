...
- **0.9.7** ✅ (this release) — `TE: trailers` request signaling on both 1.1 and h2 paths; h2 forbidden filter loosened to allow `te: trailers` (the one value RFC 7540 §8.1.2.2 permits) while still dropping other TE values like `te: gzip`.
