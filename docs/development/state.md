...
**1.6.4** — 2026-06-17. **Binary streaming download — `sandhi_http_download` / `_download_sink`; cyrius pin `6.2.11 → 6.2.18`.** First consumer: **takumi** source download (takumi `docs/adr/0006-source-download.md`); promotes the parked *Streaming GET to an fd / body-sink* roadmap item (a direct consumer ask is the trigger — overrides wait-for-second-consumer). The buffered `sandhi_http_get_opts` allocates the whole body up front (takumi capped tarballs at 128 MiB to dodge bump-allocator exhaustion), and `sandhi_http_stream*` only carries SSE (`text/event-stream`), so a binary artifact had no incremental path. New module `src/http/download.cyr` adds the third shape: stream a binary body to a caller byte-sink (`sandhi_http_download_sink(url, cb, ctx, opts)`; `cb(ctx, buf, len)` returns 1=continue / 0=clean-stop / <0=abort) or straight to an fd (`sandhi_http_download(url, fd, opts)`), **never fully buffering** — decoded bytes flush to the sink each pass and the fixed-size work buffers reset, so resident memory is bounded and the 128 MiB cap lifts. **Composes, doesn't fork**: reuses `stream.cyr`'s header drain + chunked decoder + buffer helpers and `client.cyr`'s security-aware redirect-follow (https→http downgrade → `SANDHI_ERR_TLS`; cred-strip across authorities), honoring the same timeout matrix + TLS policy. Only a **2xx** final streams to the sink; any other final status returns its code with `err_kind=REMOTE` and writes nothing (no error-page splatter). +8 public verbs (`_download_sink[_a]`, `_download[_a]`, `sandhi_download_status`/`_bytes`/`_err`/`_stopped`). **No new cyrius primitive** — pure composition. **Verified**: **1039 assertions green** (467 + 167 + 342 + 63; sandhi 451 → 467: download-result accessors, fd-sink temp-file round-trip + write-error signal, redirect-target resolver across 302/200/follow-off/hops-exhausted/no-Location/downgrade-sentinel); DCE-parity build OK; lint 0/0; `dist/sandhi.cyr` regenerated at v1.6.4.
...
**1039 assertions green** across four suites (CI runs all four; measured on the 6.2.18 pin at 1.6.4):

- `tests/sandhi.tcyr` — **467** — headers / URL / response / client + redirect security (cred-strip cross-authority, https→http refusal, 303→GET) / DNS / discovery (incl. the 1.5.5 mDNS QM wire checks + a loopback live receive round-trip) / TLS policy + fingerprint (incl. the 1.6.0 native trust/mTLS enforcement-available + pin-available flips) / SSE / streaming / **download** (1.6.4: result accessors, fd-sink temp-file round-trip + write-error signal, redirect-target resolver across 302/200/follow-off/hops-exhausted/no-Location/downgrade-sentinel).
