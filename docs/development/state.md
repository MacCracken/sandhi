...
- **0.9.10** ✅ (this release) — `_sandhi_pool_has_idle` stale-skip hardening. Pool peek now ignores conns past `idle_timeout_ms`, so ALPN promotion fires on routes whose only 1.1 entries are stale.
