...
| Doc | Crate | Side | State (verified 2026-09-23) |
|-----|-------|------|------------------------------|
| [`2026-04-24-daimon-registry-endpoints.md`](2026-04-24-daimon-registry-endpoints.md) | daimon | producer | **Not implemented.** daimon 2.4.2 serves `/v1/*` and has no `/services/` route; sandhi's resolver calls `{base}/services/{name}` → 404. Namespace decision needed on one side or the other — the doc now offers `/v1/services/{name}` (sandhi re-points one URL builder). |
...
| Doc | Crate | Evidence |
|-----|-------|----------|
| [`archive/2026-08-23-bote-ssrf-needs-preresolved-client-connect.md`](archive/2026-08-23-bote-ssrf-needs-preresolved-client-connect.md) | bote 3.3.7 | **Resolved in sandhi 1.9.14** — `sandhi_client_set_resolver` (shape B, the resolve hook). Chosen over the pre-resolved `_at` entry point because the hook fires **per redirect hop by construction**, which is the caveat the filing flagged as easy to miss and silently fatal. Gated by `programs/_ssrf_resolver_gate.cyr`. Reached consumers at **cyrius 6.5.37** (sandhi 1.9.15 fold) — any bote pin ≥ 6.5.37 has the hook. |
...
what was actually measured rather than assumed. The 2026-09-23 sweep also repaired their
paste-ready handoffs for the post-fold world: `include "lib/sandhi.cyr"` with `"sandhi"` in
`[deps] stdlib` (there is no `[deps.sandhi]` pin to bump), and caveats that told consumers
to wait for features that have since shipped (SSE, `max_response_bytes`, streaming
download). Every verb in every migration example was checked against the current surface. sandhi's side is shipped in every
...
> `dist/sandhi.cyr` — there is no sandhi pin left to bump. For the fixes archived so far
> that re-vendor **has landed** (checked 2026-09-23 against the cyrius CHANGELOG + the
> installed snapshots): cyrius **6.5.6** folded sandhi 1.9.9 (the 1.9.8 accept-spin fix),
> **6.5.37** folded 1.9.15 (the 1.9.12 P1s, bote's 1.9.14 resolver hook, the 1.9.15 SSE
> fix), and every 6.6.x snapshot vendors ≥ 1.9.16. A consumer pinned *below* those
> releases still carries the bug — the caveat is about the pin, not the archive.
...
| [`archive/2026-06-28-h2-promote-v6-conn-open-arity.md`](archive/2026-06-28-h2-promote-v6-conn-open-arity.md) | sandhi 1.7.0 | yeo-cy-test: the h2-promote **IPv6** branch in `dispatch.cyr` still called `_sandhi_conn_open_v6_fully_timed_a` with 8 args after 1.6.9 added its trailing `ctx` (9) — a build warning in every consumer of the folded bundle, and a garbage `ctx` read at runtime on the IPv6 + h2-promote path. **Fixed** by passing `0` (the ctx==0 fallback). *(Indexed 2026-09-23 — archived at 1.7.0 but missing from this table.)* |
| [`archive/2026-06-28-pooled-tls-misleading-concurrency-comment.md`](archive/2026-06-28-pooled-tls-misleading-concurrency-comment.md) | sandhi 1.7.0 (doc/gate) + 1.8.1 (gating) | yeo-cy-test: `sandhi_server_run_pooled_tls`'s header claimed concurrent TLS handshakes were safe; they SIGSEGV'd on sigil's process-global crypto scratch. **1.7.0** corrected the comment (`max_conns = 1`) and added the `[4]` concurrent-handshake watch to `_server_tls_probe.cyr` (0/16 then); **1.8.1** promoted `[4]` to gating once sigil's per-thread crypto banking landed (16/16) and relaxed the guidance. *(Indexed 2026-09-23 — archived at 1.7.0 but missing from this table.)* |
