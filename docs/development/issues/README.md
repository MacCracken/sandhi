...
|-----|------|----------|---------|
| [`2026-08-23-cyrius-distlib-deps-scan-reads-comments.md`](2026-08-23-cyrius-distlib-deps-scan-reads-comments.md) | cyrius | low (noise, not breakage) | `cyrius distlib` derives the `.deps` leaf requirements by scanning identifiers in **comment text** as well as code. Adding a comment containing the bare word `assert` to `src/http/conn.cyr` — no code change — added `assert` to `dist/sandhi-tls.deps` and `dist/sandhi-server.deps`; changing that one word to `check` removed it. Over-declares only, so nothing fails to link, but it makes the sidecars churn on prose edits and weakens the CI drift gate. Suggested fix: strip `#`-to-EOL before the scan. |
