...
- [004 — Native TLS is the default; libssl is a deprecated opt-in](004-native-tls-default.md) — native is the **no-flag default** since Cyrius 6.1.21 / sandhi 1.4.9 (`-D CYRIUS_TLS_LIBSSL` opts out to the deprecated bridge; legacy `-D CYRIUS_TLS_NATIVE` is a no-op alias). Why native (libssl's glibc-malloc fought cyrius's brk heap → 4th-request SIGSEGV, fixed at Cyrius 6.1.19), the `sandhi_tls_use_libssl()` escape hatch, and the one remaining libssl coupling: trust-store / mTLS policy *enforcement* (SPKI pinning is already backend-agnostic).
