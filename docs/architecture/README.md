...
- [005 — aarch64 cross-build is best-effort](005-aarch64-bayan-cross-build.md) — since 1.4.11, `cycc_aarch64` aborts with `unexpected enum` assembling stdlib `bayan` (sigil's transitive dep). Upstream `cycc_aarch64` dep-assembly defect, not sandhi's — reproducible with zero sandhi code, affects every toolchain 6.0.21–6.2.1, x86_64 unaffected. CI/release tolerate it (warn + skip the artifact); the x86_64 build is authoritative.
